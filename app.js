...
window.editEmployee=async id=>{ const e=state.employees.find(x=>x.id===id); const name=prompt('Employee name',e.name||''); if(!name) return; const department=prompt('Department',e.department||''); const role=prompt('Role',e.role||''); const paidLeaves=prompt('Paid leaves/month',e.paidLeaves||state.settings.monthlyLeaves||2); const photoUrl=prompt('Profile photo URL',e.photoUrl||''); await updateDoc(doc(db,'employees',id),{name,department,role,paidLeaves,photoUrl}); toast('Employee updated'); await refresh(); };
...
window.rejectLeave=async id=>{ const l=state.leaveRequests.find(x=>x.id===id); if(!l)return; const note=prompt('Reject reason','Not approved')||'Not approved'; await updateDoc(doc(db,'leave_requests',id),{status:'rejected',adminNote:note,rejectedAt:serverTimestamp()}); await logActivity(l.empId,`Leave rejected: ${note}`,'leave'); toast('Leave rejected'); await refresh(); };
