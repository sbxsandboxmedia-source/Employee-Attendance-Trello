...
async function uploadFiles(files, taskCode, type){
  const arr=[]; const list=[...files||[]];
  for(const file of list){
    const path=`task-files/${taskCode||'task'}/${type}/${Date.now()}-${safeFileName(file.name)}`;
    const storageRef=ref(storage,path);
    await uploadBytes(storageRef,file);
    const url=await getDownloadURL(storageRef);
    arr.push({name:file.name,url,path,type:file.type||'',size:file.size,kind:fileKind(file.name),uploadedBy:state.role==='admin'?'Admin':state.employee?.name||state.employee?.empId||'Employee',uploadedAt:`${today()} ${nowTime()}`});
  }
  return arr;
}
...
function commentsHTML(t){
  const c=t.comments||[];
  return c.length ? c.slice().reverse().map(x=>`<div class="comment"><b>${esc(x.by)}</b><small>${esc(x.date)} ${esc(x.time)}</small><p>${esc(x.text)}</p></div>`).join('') : '<p class="muted-small">No comments yet</p>';
}
...
function statCards(){const rec=todayRecords(); const present=rec.filter(r=>['Present','Break','Exited'].includes(r.status)).length; const review=state.tasks.filter(t=>t.status==='review').length; return `<div class="grid cards"><div class="card stat"><p>Total Employees</p><h3>${state.employees.length}</h3></div><div class="card stat"><p>Present Today</p><h3>${present}</h3></div><div class="card stat"><p>On Break</p><h3>${rec.filter(r=>r.status==='Break').length}</h3></div><div class="card stat"><p>Waiting Review</p><h3>${review}</h3></div></div>`}
function adminDashboard(){
  ...
  $('#adminDashboard').innerHTML = `${statCards()}<div class="grid two" style="margin-top:18px"><div class="card"><h3>Task Status Graph</h3><div class="chart">${['todo','progress','review','complete'].map(s=>`<div class="bar" title="${s}" style="height:${20+state.tasks.filter(t=>t.status===s).length*28}px"></div>`).join('')}</div></div><div class="card"><h3>Task Report</h3><p class="notice">${esc(state.settings.announcement||'')}</p><h2>${completed}/${state.tasks.length}</h2><p>Tasks completed</p></div></div><div class="grid two" style="margin-top:18px"><div class="card"><h3>Live Team Status</h3>${live}</div><div class="card"><h3>Recent Task Activity</h3>${state.activity.filter(a=>a.date===today()).slice(-8).reverse().map(a=>`<div class="timeline-item"><b>${a.time}</b><span>${esc(empName(a.empId))} — ${esc(a.text)}</span></div>`).join('')||'<p>No activity today</p>'}</div></div>`;
}
function employeesView(){
  const depOpts = state.departments.map(d=>`<option>${esc(d.name)}</option>`).join('');
  $('#employeesView').innerHTML = `<div class="card"><h3>Add Employee</h3><form id="empForm" class="form-grid"><input name="name" placeholder="Employee name" required><input name="empId" placeholder="Employee ID e.g. EMP001" required><input name="password" placeholder="Password" required><select name="department">${depOpts}</select><input name="role" placeholder="Role"><input name="phone" placeholder="Phone"><button class="primary-btn">Add Employee</button></form></div><div class="card"><h3>Employees</h3><div class="table-wrap"><table class="table"><tr><th>Name</th><th>ID</th><th>Department</th><th>Status</th><th>Live</th><th>Action</th></tr>${state.employees.map(e=>{const st=liveStatus(e.empId);return `<tr><td>${esc(e.name)}</td><td>${esc(e.empId)}</td><td>${esc(e.department||'-')}</td><td><span class="status ${currentStatus(e.empId).toLowerCase()}">${currentStatus(e.empId)}</span></td><td><span class="status ${st.cls}">${esc(st.label)}</span></td><td><button class="ghost-btn" onclick="window.editEmployee('${e.id}')">Edit</button> <button class="danger-btn" onclick="window.delEmployee('${e.id}')">Delete</button></td></tr>`}).join('')}</table></div></div>`;
  $('#empForm').onsubmit=async ev=>{ev.preventDefault(); const data=Object.fromEntries(new FormData(ev.target)); await addDoc(collection(db,'employees'),data); toast('Employee added'); await refresh();};
}
window.delEmployee=async id=>{ if(confirm('Delete employee?')){await deleteDoc(doc(db,'employees',id)); toast('Employee deleted'); await refresh();} };
window.editEmployee=async id=>{ const e=state.employees.find(x=>x.id===id); const name=prompt('Employee name',e.name||''); if(!name) return; const department=prompt('Department',e.department||''); const role=prompt('Role',e.role||''); await updateDoc(doc(db,'employees',id),{name,department,role}); toast('Employee updated'); await refresh(); };
function departmentsView(){
  ...
  $('#depForm').onsubmit=async ev=>{ev.preventDefault(); await addDoc(collection(db,'departments'),Object.fromEntries(new FormData(ev.target))); toast('Department added'); await refresh();};
}
window.delDepartment=async id=>{ if(confirm('Delete department?')){await deleteDoc(doc(db,'departments',id)); toast('Department deleted'); await refresh();} };
window.editDepartment=async id=>{ const d=state.departments.find(x=>x.id===id); const name=prompt('Department name',d.name); if(!name)return; await updateDoc(doc(db,'departments',id),{name}); toast('Department updated'); await refresh(); };
...
window.moveTask=async (id,status)=>{
  ...
  if(status==='complete') { extra.completedAt=serverTimestamp(); extra.completedBy='admin'; const rating=prompt('Rating 1-5 for this work','5')||''; const reviewComment=prompt('Admin review comment','Approved')||''; extra.rating=rating; extra.reviewComment=reviewComment; extra.history.push(historyItem(`Approved with ${rating} star rating: ${reviewComment}`,'Admin')); }
...
window.viewTask=(id)=>{ const t=state.tasks.find(x=>x.id===id); if(!t)return; const modal=document.createElement('div'); modal.className='modal-backdrop'; modal.innerHTML=`<div class="modal-card"><button class="modal-close" onclick="this.closest('.modal-backdrop').remove()">×</button><h2>${esc(t.taskCode)} — ${esc(t.title)}</h2><div class="task-meta"><span class="status ${priorityClass(t.priority)}">${esc(t.priority)}</span><span class="mini">${esc(t.business)}</span><span class="mini">${esc(t.department)}</span><span class="mini">Assigned: ${esc(empName(t.empId))}</span><span class="mini">Due: ${dateNice(t.dueDate)}</span></div><p class="notice">${esc(t.description||'No description')}</p><div class="modal-grid"><div><h3>Admin Reference Files</h3>${fileListHTML(t.referenceFiles||[])}</div><div><h3>Final Work / Versions</h3>${workVersionsHTML(t)}</div></div><div class="modal-grid"><div><h3>Comments</h3>${commentsHTML(t)}<button class="ghost-btn" onclick="window.addTaskNote('${t.id}')">Add Comment</button></div><div><h3>Timeline</h3>${taskHistoryHTML(t)}</div></div>${t.rating?`<div class="notice"><b>Admin Rating:</b> ${esc(t.rating)} ⭐<br><b>Review:</b> ${esc(t.reviewComment||'')}</div>`:''}</div>`; document.body.appendChild(modal); };
function chooseFiles(cb){ const input=document.createElement('input'); input.type='file'; input.multiple=true; input.onchange=()=>cb(input.files); input.click(); }
window.uploadReference=(id)=>chooseFiles(async files=>{ const t=state.tasks.find(x=>x.id===id); if(!t||!files.length)return; const uploaded=await uploadFiles(files,t.taskCode,'reference'); await updateDoc(doc(db,'tasks',id),{referenceFiles:[...(t.referenceFiles||[]),...uploaded],history:[...(t.history||[]),historyItem(`${uploaded.length} reference file uploaded`)],updatedAt:serverTimestamp()}); await logActivity(t.empId,`Reference uploaded: ${t.taskCode}`,'task'); toast('Reference uploaded'); await refresh(); });
window.uploadWork=(id)=>chooseFiles(async files=>{ const t=state.tasks.find(x=>x.id===id); if(!t||!files.length)return; const uploaded=await uploadFiles(files,t.taskCode,'work'); let version=(t.workFiles||[]).length+1; uploaded.forEach(f=>f.version=version++); await updateDoc(doc(db,'tasks',id),{workFiles:[...(t.workFiles||[]),...uploaded],history:[...(t.history||[]),historyItem(`${uploaded.length} final work file uploaded`)],updatedAt:serverTimestamp()}); await logActivity(t.empId,`Work uploaded: ${t.taskCode}`,'task'); toast('Work uploaded'); await refresh(); });
window.requestRevision=async(id)=>{ const t=state.tasks.find(x=>x.id===id); const note=prompt('Revision note / changes needed'); if(!note)return; await updateDoc(doc(db,'tasks',id),{status:'progress',revisionNote:note,history:[...(t.history||[]),historyItem(`Revision requested: ${note}`,'Admin')],updatedAt:serverTimestamp()}); await logActivity(t.empId,`Revision requested: ${t.taskCode}`,'task'); toast('Task sent back to Progress'); await refresh(); };
...
function requestsView(){ $('#requestsView').innerHTML = `<div class="card"><h3>Messages / Leave Requests</h3>${state.activity.filter(a=>['message','leave'].includes(a.type)).slice().reverse().map(a=>`<div class="timeline-item"><b>${a.date}</b><span>${esc(empName(a.empId))}: ${esc(a.text)}</span></div>`).join('')||'<p>No requests yet</p>'}</div>`; }
function settingsView(){
  $('#settingsView').innerHTML = `<div class="card"><h3>Office Settings</h3><form id="settingsForm" class="form-grid"><input name="start" value="${esc(state.settings.start||'10:00')}" placeholder="Start time"><input name="end" value="${esc(state.settings.end||'19:00')}" placeholder="End time"><input name="graceMinutes" value="${esc(state.settings.graceMinutes||10)}" placeholder="Grace minutes"><input name="breakMinutes" value="${esc(state.settings.breakMinutes||45)}" placeholder="Break minutes"><input name="monthlyLeaves" value="${esc(state.settings.monthlyLeaves||2)}" placeholder="Monthly leaves"><input name="weekend" value="${esc(state.settings.weekend||'Sunday')}" placeholder="Weekend"><textarea name="announcement" placeholder="Announcement">${esc(state.settings.announcement||'')}</textarea><button class="primary-btn">Save Settings</button></form></div>`;
