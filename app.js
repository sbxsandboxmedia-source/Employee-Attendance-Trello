...
async function loadOnce() {
  const [emps, deps, tasks, atts, leaves, acts, spaces, workflows] = await Promise.all([
    getDocs(collection(db, "employees")), getDocs(collection(db, "departments")), getDocs(collection(db, "tasks")), getDocs(collection(db, "attendance")), getDocs(collection(db, "leave_requests")), getDocs(collection(db, "activity_logs")), getDocs(collection(db, "workflow_spaces")), getDocs(collection(db, "workflow_templates"))
  ]);
  ...
  state.spaces = spaces.docs.map((d) => ({ id: d.id, ...d.data() }));
  state.workflows = workflows.docs.map((d) => ({ id: d.id, ...d.data() }));
...
function subscribeData() {
  ...
  listen("workflow_spaces", (v) => state.spaces = v);
  listen("workflow_templates", (v) => state.workflows = v);
...
function navItems() {
  ...
  return ["dashboard", "employees", "departments", "tasks", "workflows", "attendance", "requests", "payroll", "settings"];
}

function viewTitle(v) { return ({ dashboard: "Dashboard", employees: "Employees", departments: "Departments", tasks: "Tasks", workflows: "Workflows", attendance: "Attendance", requests: "Requests", payroll: "Payroll", settings: "Settings", history: "Completed History" }[v] || v); }
...
function renderView() {
  if (!$("#view")) return;
  const map = { dashboard: dashboardView, employees: employeesView, departments: departmentsView, tasks: tasksView, workflows: workflowsView, attendance: attendanceView, requests: requestsView, payroll: payrollView, settings: settingsView, history: completedHistoryView };
...
function spaceName(id) { return state.spaces.find((s) => s.id === id)?.name || "No Space"; }
function workflowById(id) { return state.workflows.find((w) => w.id === id); }
function workflowStepSummary(w) {
  const steps = Array.isArray(w.steps) ? w.steps : [];
  return steps.map((st, i) => `<div class="workflow-step"><span>${i + 1}</span><div><b>${esc(st.title || `Step ${i + 1}`)}</b><small>${esc(st.department || "Department")} → ${esc(empName(st.empId) || "Employee")}</small></div></div>`).join("") || `<p class="muted">No steps added</p>`;
}
function stepRowsHtml() {
  return Array.from({ length: 5 }, (_, i) => `<div class="workflow-step-form">
    <b>Step ${i + 1}</b>
    <input name="stepTitle${i}" placeholder="Task title / work name">
    <select name="stepDepartment${i}"><option value="">Department</option>${deptOptions()}</select>
    <select name="stepEmpId${i}"><option value="">Assign employee</option>${empOptions()}</select>
    <textarea name="stepDescription${i}" placeholder="Instruction for this step"></textarea>
  </div>`).join("");
}
function workflowsView() {
  if (state.role !== "admin") return `<div class="card">Access denied</div>`;
  return `<div class="cards workflow-kpis"><div class="card kpi-card green"><p>Total Spaces</p><h3>${state.spaces.length}</h3><small>ClickUp style work areas</small></div><div class="card kpi-card blue"><p>Workflow Templates</p><h3>${state.workflows.length}</h3><small>Auto department chain</small></div><div class="card kpi-card purple"><p>Running Workflow Tasks</p><h3>${state.tasks.filter(t=>t.workflowId && t.status!=="complete").length}</h3><small>Active steps</small></div><div class="card kpi-card orange"><p>Completed Workflow Tasks</p><h3>${state.tasks.filter(t=>t.workflowId && t.status==="complete").length}</h3><small>All time</small></div></div>
  <div class="grid two" style="margin-top:18px"><div class="card"><div class="section-title"><h3>Create Space</h3><span class="badge blue">Like ClickUp Space</span></div><form id="spaceForm" class="form-grid"><input name="name" placeholder="Space name e.g. SIA Instagram" required><input name="company" placeholder="Company / Client"><textarea name="description" class="wide" placeholder="Space description"></textarea><button class="btn primary">Create Space</button></form></div>
  <div class="card"><h3>Spaces</h3>${state.spaces.map(s=>`<div class="leader"><div><b>${esc(s.name)}</b><div class="muted">${esc(s.company||"")} ${s.description?"• "+esc(s.description):""}</div></div><button class="btn small danger" data-del-space="${s.id}">Delete</button></div>`).join("") || `<p class="muted">No space created yet.</p>`}</div></div>
  <div class="card" style="margin-top:18px"><div class="section-title"><div><h3>Create Workflow Template</h3><p class="muted" style="margin:4px 0 0">Admin yaha department chain set karega. Ek step approve hote hi next department ko task automatic assign hoga.</p></div></div><form id="workflowForm" class="workflow-form"><div class="form-grid"><input name="name" placeholder="Workflow name e.g. Instagram Post Process" required><select name="spaceId"><option value="">Select Space</option>${state.spaces.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join("")}</select><input name="business" placeholder="Company / Client name"><textarea name="description" class="wide" placeholder="Workflow description"></textarea></div><h4>Workflow Steps</h4>${stepRowsHtml()}<button class="btn primary">Save Workflow</button></form></div>
  <div class="card" style="margin-top:18px"><h3>Workflow Templates</h3><div class="workflow-list">${state.workflows.map(w=>`<div class="workflow-card"><div class="workflow-card-head"><div><h3>${esc(w.name)}</h3><p class="muted">Space: ${esc(spaceName(w.spaceId))} ${w.business?"• "+esc(w.business):""}</p></div><div class="row-actions"><button class="btn small primary" data-launch-workflow="${w.id}">Start Workflow</button><button class="btn small danger" data-del-workflow="${w.id}">Delete</button></div></div><div class="workflow-steps">${workflowStepSummary(w)}</div></div>`).join("") || `<p class="muted">No workflow template yet.</p>`}</div></div>`;
}
...
function bindViewActions() {
  ...
  const spaceForm = $("#spaceForm"); if (spaceForm) spaceForm.onsubmit = addSpace;
  const workflowForm = $("#workflowForm"); if (workflowForm) workflowForm.onsubmit = addWorkflow;
  ...
  document.querySelectorAll("[data-launch-workflow]").forEach((b)=>b.onclick=()=>launchWorkflow(b.dataset.launchWorkflow));
  document.querySelectorAll("[data-del-workflow]").forEach((b)=>b.onclick=()=>delDoc("workflow_templates", b.dataset.delWorkflow, "Delete workflow?"));
  document.querySelectorAll("[data-del-space]").forEach((b)=>b.onclick=()=>delDoc("workflow_spaces", b.dataset.delSpace, "Delete space?"));
...
async function addSpace(e) {
  ...
  await addDoc(collection(db, "workflow_spaces"), { ...data, createdAt: todayISO() });
  e.target.reset();
  toast("Space created");
}
async function addWorkflow(e) {
  ...
  const steps = [];
  for (let i = 0; i < 5; i++) {
    const title = (data[`stepTitle${i}`] || "").trim();
    const department = (data[`stepDepartment${i}`] || "").trim();
    const empId = (data[`stepEmpId${i}`] || "").trim();
    const description = (data[`stepDescription${i}`] || "").trim();
    if (title || department || empId || description) {
      if (!title || !department || !empId) throw new Error(`Step ${i + 1} me title, department aur employee select karo`);
      steps.push({ title, department, empId, description });
    }
    delete data[`stepTitle${i}`]; delete data[`stepDepartment${i}`]; delete data[`stepEmpId${i}`]; delete data[`stepDescription${i}`];
  }
  if (!steps.length) throw new Error("Minimum 1 workflow step add karo");
  await addDoc(collection(db, "workflow_templates"), { ...data, steps, createdAt: todayISO(), updatedAt: todayISO() });
  e.target.reset();
  toast("Workflow saved");
}
async function launchWorkflow(id) {
  const w = workflowById(id);
  if (!w || !Array.isArray(w.steps) || !w.steps.length) return toast("Workflow steps missing", "error");
  const st = w.steps[0];
  const codePrefix = String(w.business || state.settings.companyCode || "WF").replace(/[^a-zA-Z0-9]/g, "").slice(0, 3).toUpperCase() || "WF";
  await addDoc(collection(db, "tasks"), {
    title: st.title,
    business: w.business || spaceName(w.spaceId),
    ...
    estimatedHours: "",
    referenceLink: "",
    description: st.description || w.description || "",
    taskCode: `${codePrefix}-WF-${String(state.tasks.length + 1).padStart(4, "0")}`,
    status: "todo",
    rating: 0,
    points: 0,
    workflowId: w.id,
  ...
  toast("Workflow started. First task assigned.");
}
async function createNextWorkflowTask(task) {
  if (!task.workflowId && task.workflowId !== 0) return;
  const w = workflowById(task.workflowId);
  if (!w || !Array.isArray(w.steps)) return;
  const nextIndex = Number(task.stepIndex || 0) + 1;
  if (nextIndex >= w.steps.length) return;
  const already = state.tasks.some((t) => t.workflowId === task.workflowId && Number(t.stepIndex || 0) === nextIndex && t.parentTaskId === task.id);
  if (already) return;
  const st = w.steps[nextIndex];
  const codePrefix = String(w.business || state.settings.companyCode || "WF").replace(/[^a-zA-Z0-9]/g, "").slice(0, 3).toUpperCase() || "WF";
  await addDoc(collection(db, "tasks"), {
    title: st.title,
    business: task.business || w.business || spaceName(w.spaceId),
    department: st.department,
    empId: st.empId,
    ...
    description: st.description || `Auto assigned after ${task.taskCode || task.title} approval`,
    taskCode: `${codePrefix}-WF-${String(state.tasks.length + 1).padStart(4, "0")}`,
    status: "todo",
    rating: 0,
    points: 0,
