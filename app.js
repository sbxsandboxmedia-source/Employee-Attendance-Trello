...
window.moveTask=async (id,status)=>{
  ...
  if(status==='complete') { extra.completedAt=serverTimestamp(); extra.completedBy='admin'; let rating=prompt('Rating 1-5 for this work','5')||'5'; rating=String(Math.max(1,Math.min(5,Number(rating)||5))); const reviewComment=prompt('Admin review comment','Approved')||'Approved'; extra.rating=rating; extra.points=pointsForRating(rating); extra.reviewComment=reviewComment; extra.history.push(historyItem(`Approved with ${rating} star rating (${extra.points} points): ${reviewComment}`,'Admin')); }
...
window.editTask=async id=>{ const t=state.tasks.find(x=>x.id===id); const title=prompt('Task title',t.title||''); if(!title)return; const priority=prompt('Priority Low/Medium/High/Urgent',t.priority||'Medium'); const dueDate=prompt('Due date YYYY-MM-DD',t.dueDate||''); const description=prompt('Description',t.description||''); const referenceLink=prompt('Reference Link',t.referenceLink||''); await updateDoc(doc(db,'tasks',id),{title,priority,dueDate,description,referenceLink,updatedAt:serverTimestamp()}); await logActivity(t.empId,`Task edited: ${t.taskCode}`,'task'); toast('Task edited'); await refresh(); };
...
function employeeSummaryView(){ const my=state.employee.empId; const rows=state.attendance.filter(a=>a.empId===my); const tasks=state.tasks.filter(t=>t.empId===my); const completed=tasks.filter(t=>t.status==='complete'); const points=tasks.reduce((sum,t)=>sum+Number(t.points||pointsForRating(t.rating)),0); const avg=completed.length?(completed.reduce((s,t)=>s+Number(t.rating||0),0)/completed.length).toFixed(1):'--'; $('#employeeSummaryView').innerHTML = `<div class="grid cards"><div class="card stat"><p>Total Days</p><h3>${rows.length}</h3></div><div class="card stat"><p>Completed Tasks</p><h3>${completed.length}</h3></div><div class="card stat"><p>Work Points</p><h3>${points}</h3></div><div class="card stat"><p>Average Rating</p><h3>${avg} ⭐</h3></div></div><div class="card"><h3>Completed Task Ratings</h3>${completed.map(t=>`<div class="live-row"><b>${esc(t.taskCode)} — ${esc(t.title)}</b><span class="status complete">${esc(t.rating||'-')}/5 • ${Number(t.points||pointsForRating(t.rating))} pts</span></div>`).join('')||'<p>No rated task yet</p>'}</div><div class="card"><h3>Monthly Graph</h3><div class="chart">${rows.slice(-20).map(r=>`<div class="bar" style="height:${r.exit?120:60}px" title="${r.date}"></div>`).join('')}</div></div>`; }
