...
function payrollCalc(emp, m=currentMonth()){ const salary=Number(emp?.salary||0); const workingDays=Number(state.settings.workingDays||26); const bonusPerTask=Number(state.settings.taskBonus||500); const completed=monthlyCompletedTasks(emp.empId,m).length; const bonus=completed*bonusPerTask; const absent=monthlyAbsentDays(emp.empId,m); const deduction= workingDays ? (salary/workingDays)*absent : 0; return {salary,workingDays,bonusPerTask,completed,bonus,absent,deduction,net:salary+bonus-deduction}; }
...
function payrollView(){
  ...
  const rows = state.employees.map(e=>({e, p: payrollCalc(e,m), pts: monthlyTaskPoints(e.empId)}));
  ...
  $('#payrollView').innerHTML = `<div class="grid cards"><div class="card stat"><p>Total Base Salary</p><h3>${money(totalBase)}</h3></div><div class="card stat"><p>This Month Bonus</p><h3>${money(totalBonus)}</h3></div><div class="card stat"><p>Leave Deduction</p><h3>${money(totalDed)}</h3></div><div class="card stat"><p>Net Payable</p><h3>${money(totalNet)}</h3></div></div><div class="card"><h3>Payroll Dashboard — ${esc(m)}</h3><p class="notice">Base salary every month same rahegi. Bonus aur deduction current month ke hisaab se fresh calculate hoga. Task complete bonus: ${money(state.settings.taskBonus||500)} per completed task. Extra leave/absent deduction salary ÷ ${esc(state.settings.workingDays||26)} working days.</p><div class="table-wrap"><table class="table"><tr><th>Employee</th><th>Department</th><th>Base Salary</th><th>Completed Tasks</th><th>Bonus</th><th>Absent / Extra Leave</th><th>Deduction</th><th>Net Salary</th><th>Points</th></tr>${rows.map(x=>`<tr><td>${avatarHTML(x.e,true)} ${esc(x.e.name)}</td><td>${esc(x.e.department||'-')}</td><td>${money(x.p.salary)}</td><td>${x.p.completed}</td><td><span class="status complete">+ ${money(x.p.bonus)}</span></td><td>${x.p.absent}</td><td><span class="status absent">- ${money(x.p.deduction)}</span></td><td><b>${money(x.p.net)}</b></td><td>${x.pts}</td></tr>`).join('') || '<tr><td colspan="9">No employees</td></tr>'}</table></div></div>`;
}
function settingsView(){
  $('#settingsView').innerHTML = `<form id="settingsForm" class="settings-grid"><div class="card settings-card"><h3>Office Timing</h3><p class="muted-small">Yaha office start/end aur break rules set karo.</p><label>Office Start Time<input name="start" value="${esc(state.settings.start||'10:00')}" placeholder="10:00"></label><label>Office End Time<input name="end" value="${esc(state.settings.end||'19:00')}" placeholder="19:00"></label><label>Grace Minutes<input name="graceMinutes" type="number" value="${esc(state.settings.graceMinutes||10)}" placeholder="10"></label><label>Break Minutes<input name="breakMinutes" type="number" value="${esc(state.settings.breakMinutes||45)}" placeholder="45"></label></div><div class="card settings-card"><h3>Leave Rules</h3><p class="muted-small">Sunday automatic leave rahega. Extra approved leave absent count hoga.</p><label>Default Paid Leave / Month<input name="monthlyLeaves" type="number" value="${esc(state.settings.monthlyLeaves||2)}" placeholder="2"></label><label>Weekly Off<input name="weekend" value="${esc(state.settings.weekend||'Sunday')}" placeholder="Sunday"></label><label>Payroll Working Days<input name="workingDays" type="number" value="${esc(state.settings.workingDays||26)}" placeholder="26"></label></div><div class="card settings-card"><h3>Payroll & Bonus</h3><p class="muted-small">Har new month bonus/deduction fresh calculate hoga.</p><label>Task Complete Bonus ₹<input name="taskBonus" type="number" value="${esc(state.settings.taskBonus||500)}" placeholder="500"></label><label>Company Logo URL<input name="logoUrl" value="${esc(state.settings.logoUrl||'')}" placeholder="https://..."></label></div><div class="card settings-card wide"><h3>Announcement</h3><p class="muted-small">Admin dashboard par show hoga.</p><label>Announcement Message<textarea name="announcement" placeholder="Announcement">${esc(state.settings.announcement||'')}</textarea></label><button class="primary-btn">Save Settings</button></div></form>`;
