...
window.assignTask = async function () {
  ...
  const taskData = {
    ...
    workflowId: oldTask?.workflowId || "",
    workflowName: oldTask?.workflowName || "",
    workflowRunId: oldTask?.workflowRunId || "",
    workflowProjectTitle: oldTask?.workflowProjectTitle || "",
    workflowStepIndex: oldTask?.workflowStepIndex ?? null,
    workflowTotalSteps: oldTask?.workflowTotalSteps ?? null,
    updatedAt: nowISO()
  };
...
window.markTaskDone = async function (id) {
  ...
  if (task?.workflowId) {
    await createNextWorkflowTask(task);
  }
  renderMyTasks();
  renderAchievements();
  renderRewards();
  ...
  return String(raw || "")
    ...
    .map((line, index) => {
      ...
      return {
        ...
        taskText: parts[2] || `${parts[0] || "Department"} work`,
        priority: parts[3] || "Medium",
        dueDays: Number(parts[4] || 1)
      };
    });
}

function employeeByStep(step) {
  if (!step) return null;
  return employees.find(e => e.code === step.employeeCode) ||
    employees.find(e => e.name?.toLowerCase() === step.employeeCode?.toLowerCase()) ||
    employees.find(e => (e.departments || [e.department || ""]).includes(step.department));
}
...
window.saveWorkflow = async function () {
  ...
  const space = $("workflowSpace")?.value.trim();
  const prefix = $("workflowPrefix")?.value.trim() || "WF";
  const steps = parseWorkflowSteps($("workflowSteps")?.value || "");

  if (!name || !space || steps.length < 2) {
    alert("Workflow name, space/business aur minimum 2 steps zaroori hai");
    return;
  }
  ...
  ["workflowName", "workflowPrefix", "workflowSteps"].forEach(id => { if ($(id)) $(id).value = ""; });
  alert("Workflow saved. Ab is workflow ko start kar sakte ho.");
};

window.deleteWorkflow = async function(id){
  if(confirm("Workflow delete karna hai?")) await deleteDoc(doc(db, "workflows", id));
};

window.startWorkflow = async function () {
  const workflowId = $("startWorkflowSelect")?.value;
  const projectTitle = $("workflowProjectTitle")?.value.trim();
  const wf = workflows.find(w => w.id === workflowId);
  if (!wf || !projectTitle) {
    alert("Workflow aur project/task title select karo");
    return;
  }

  const first = (wf.steps || [])[0];
  const emp = employeeByStep(first);
  if (!emp) {
    alert(`Step 1 ke liye employee nahi mila. Department/Employee Code check karo: ${first.department}`);
    return;
  }

  const runId = `${wf.prefix || "WF"}-${Date.now()}`;
  await addDoc(collection(db, "tasks"), {
    empId: emp.id,
    name: emp.name,
    date: todayKey(),
    text: `${projectTitle} — ${first.taskText}`,
    business: wf.space,
    department: first.department,
    taskCode: `${wf.prefix || "WF"}-${String(first.stepNo).padStart(2, "0")}`,
    deadline: addDaysKey(first.dueDays),
    priority: first.priority || "Medium",
    status: "Pending",
    workflowId: wf.id,
    workflowName: wf.name,
    workflowRunId: runId,
    workflowProjectTitle: projectTitle,
    workflowStepIndex: 0,
    workflowTotalSteps: (wf.steps || []).length,
    createdAt: nowISO(),
    updatedAt: nowISO()
  });

  if ($("workflowProjectTitle")) $("workflowProjectTitle").value = "";
  alert("Workflow start ho gaya. Step 1 employee ko assign ho gaya.");
};

async function createNextWorkflowTask(doneTask) {
  if (!doneTask?.workflowId) return;
  const wf = workflows.find(w => w.id === doneTask.workflowId);
  if (!wf) return;

  const nextIndex = Number(doneTask.workflowStepIndex || 0) + 1;
  const nextStep = (wf.steps || [])[nextIndex];
  if (!nextStep) return;

  const alreadyCreated = tasks.some(t =>
    t.workflowRunId === doneTask.workflowRunId &&
    Number(t.workflowStepIndex) === nextIndex
  );
  if (alreadyCreated) return;

  const emp = employeeByStep(nextStep);
  if (!emp) {
    alert(`Next workflow step employee nahi mila: ${nextStep.department}. Workflow Builder me employee/department set karo.`);
    return;
  }
  ...
  await addDoc(collection(db, "tasks"), {
    ...
    name: emp.name,
    date: todayKey(),
    text: `${doneTask.workflowProjectTitle || doneTask.text} — ${nextStep.taskText}`,
    business: wf.space,
    department: nextStep.department,
    taskCode: `${wf.prefix || "WF"}-${String(nextStep.stepNo).padStart(2, "0")}`,
    deadline: addDaysKey(nextStep.dueDays),
    priority: nextStep.priority || "Medium",
    status: "Pending",
    workflowId: wf.id,
    workflowName: wf.name,
    workflowRunId: doneTask.workflowRunId,
    workflowProjectTitle: doneTask.workflowProjectTitle || doneTask.text,
    workflowStepIndex: nextIndex,
    workflowTotalSteps: (wf.steps || []).length,
    previousTaskId: doneTask.id,
    previousDepartment: doneTask.department || "-",
    createdAt: nowISO(),
    updatedAt: nowISO()
  });
}
...
function renderWorkflowBuilder() {
  if ($("startWorkflowSelect")) {
    $("startWorkflowSelect").innerHTML = `<option value="">Select Workflow</option>` + workflows.map(w =>
      `<option value="${w.id}">${w.space} / ${w.name}</option>`
    ).join("");
  }
    ...
    $("workflowList").innerHTML = workflows.map(w => `
      <div class="workflow-card">
        <div class="workflow-card-head">
          <div><b>${w.space}</b><h4>${w.name}</h4><small>Prefix: ${w.prefix || "WF"}</small></div>
          <button class="red" onclick="deleteWorkflow('${w.id}')">Delete</button>
        </div>
        <div class="workflow-steps">
          ${(w.steps || []).map((st, i) => `
            <div class="workflow-step">
              <span>${i + 1}</span>
              <strong>${st.department}</strong>
              <small>${st.employeeCode || "Auto by department"}</small>
              <p>${st.taskText}</p>
              <em>${st.priority || "Medium"} • ${st.dueDays || 1} day</em>
            </div>
          `).join("")}
        </div>
      </div>
    `).join("") || `<p class="hint">Abhi koi workflow nahi bana hai.</p>`;
  }

  if ($("workflowRunTable")) {
    const chainTasks = tasks.filter(t => t.workflowRunId);
    const runs = [...new Set(chainTasks.map(t => t.workflowRunId))];
    $("workflowRunTable").innerHTML = runs.map(runId => {
      const arr = chainTasks.filter(t => t.workflowRunId === runId).sort((a,b) => Number(a.workflowStepIndex || 0) - Number(b.workflowStepIndex || 0));
      const first = arr[0] || {};
      const done = arr.filter(t => t.status === "Done").length;
      const total = Number(first.workflowTotalSteps || arr.length || 1);
      return `<tr>
        <td>${first.workflowProjectTitle || "-"}</td>
        <td>${first.business || "-"}</td>
        <td>${first.workflowName || "-"}</td>
        <td>${done}/${total}</td>
        <td>${arr.map(t => `${t.status === "Done" ? "✅" : "⏳"} ${t.department || "-"}`).join(" → ")}</td>
      </tr>`;
    }).join("") || `<tr><td colspan="5" style="text-align:center;">Abhi koi workflow run nahi hua.</td></tr>`;
  }
}
...
window.exportBackup = function(){
  const data = { employees, records, tasks, leaves, workUpdates, notes, announcements, documentsList, payrolls, workflows, exportedAt: nowISO() };
