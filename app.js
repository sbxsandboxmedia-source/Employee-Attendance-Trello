...
function payrollCalc(emp, m=currentMonth()){
  ...
  return {salary,workingDays,bonusAmount,minTasks,minRating,completed:completedTasks.length,goodTasks:goodTasks.length,bonusEligible,bonus,absent,deduction,net:salary+bonus-deduction};
}
...
function payrollView(){
  ...
  const rows = state.employees.map(e=>({e, p: payrollCalc(e,m), pts: monthlyTaskPoints(e.empId,m)}));
  ...
  $('#payrollView').innerHTML = `<div class="grid cards"><div class="card stat"><p>Total Base Salary</p><h3>${money(totalBase)}</h3></div><div class="card stat"><p>This Month Bonus</p><h3>${money(totalBonus)}</h3></div><div class="card stat"><p>Leave Deduction</p><h3>${money(totalDed)}</h3></div><div class="card stat"><p>Net Payable</p><h3>${money(totalNet)}</h3></div></div><div class="card"><h3>Payroll Dashboard — ${esc(m)}</h3><p class="notice">Base salary same rahegi. Bonus/deduction current month me fresh calculate hoga. Bonus rule: minimum ${esc(state.settings.bonusMinTasks||10)} completed tasks with ${esc(state.settings.bonusMinRating||4)}⭐ or above = ${money(state.settings.performanceBonus||1000)} bonus.</p><div class="table-wrap"><table class="table"><tr><th>Employee</th><th>Department</th><th>Base Salary</th><th>Completed</th><th>Good Rated Tasks</th><th>Bonus</th><th>Absent / Extra Leave</th><th>Deduction</th><th>Net Salary</th><th>Points</th></tr>${rows.map(x=>`<tr><td>${avatarHTML(x.e,true)} ${esc(x.e.name)}</td><td>${esc(x.e.department||'-')}</td><td>${money(x.p.salary)}</td><td>${x.p.completed}</td><td>${x.p.goodTasks}/${x.p.minTasks} (${x.p.minRating}⭐+)</td><td><span class="status ${x.p.bonusEligible?'complete':'todo'}">${x.p.bonusEligible?'+ ':''}${money(x.p.bonus)}</span></td><td>${x.p.absent}</td><td><span class="status absent">- ${money(x.p.deduction)}</span></td><td><b>${money(x.p.net)}</b></td><td>${x.pts}</td></tr>`).join('') || '<tr><td colspan="10">No employees</td></tr>'}</table></div></div><div class="card"><div class="section-head"><div><h3>Completed Task History — ${esc(m)}</h3><p class="muted-small">Admin approved task yaha monthly list me save rahega. Month change hone par current list fresh dikhegi, purani list export ke liye Firestore me rahegi.</p></div><button class="primary-btn" onclick="window.exportCompletedTasks()">Export Excel CSV</button></div><div class="table-wrap"><table class="table"><tr><th>Date</th><th>Task Code</th><th>Task</th><th>Employee</th><th>Business</th><th>Department</th><th>Rating</th><th>Points</th><th>Admin Review</th></tr>${completedRows.map(t=>`<tr><td>${esc(t.completedDate||'-')}</td><td>${esc(t.taskCode||'-')}</td><td>${esc(t.title||'-')}</td><td>${esc(empName(t.empId))}</td><td>${esc(t.business||'-')}</td><td>${esc(t.department||'-')}</td><td>${esc(t.rating||'-')}/5</td><td>${esc(t.points||pointsForRating(t.rating))}</td><td>${esc(t.reviewComment||'')}</td></tr>`).join('') || '<tr><td colspan="9">No completed tasks this month</td></tr>'}</table></div></div>`;
}
